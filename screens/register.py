...
class RegisterScreen(BaseScreen):
    def __init__(self, parent, app):
        ...
        tk.Label(body, text="ข้อมูลโรงพยาบาล / อุปกรณ์",
                 font=thai_font(self.fs(26), "bold"), bg=CARD_COLOR, fg=TEXT_COLOR).pack(anchor="w")
        ...
        for row, (lbl_text, key) in enumerate(dev_fields):
            tk.Label(dev_form, text=lbl_text, font=thai_font(self.fs(24)), bg=CARD_COLOR,
                     fg=TEXT_COLOR, anchor="e").grid(row=row, column=0, sticky="e",
                                                     padx=(0, 10), pady=6)
            # label (view mode)
            lbl = tk.Label(dev_form, text="", font=thai_font(self.fs(24)), bg=CARD_COLOR,
            ...
            e = tk.Entry(dev_form, font=thai_font(self.fs(24)), bg=ENTRY_BG, fg=TEXT_COLOR,
                         relief="sunken", bd=2, width=34)
            e.grid(row=row, column=1, sticky="w", pady=6)
            e.grid_remove()
            self.dev_entries[key] = e

        self.dev_msg = tk.Label(body, text="", font=thai_font(self.fs(20)), bg=CARD_COLOR, fg="#cc0000")
        ...
        self._dev_btn_save = self.primary_btn(
            ...
            fontsize=self.fs(22), width=22, pady=6)
        ...
        for row, (lbl_text, key) in enumerate([("ชื่อ :", "name"), ("นามสกุล :", "lastname"), ("รหัส :", "password")]):
            tk.Label(user_form, text=lbl_text, font=thai_font(self.fs(24)), bg=CARD_COLOR,
                     fg=TEXT_COLOR, anchor="e").grid(row=row, column=0, sticky="e",
                                                     padx=(0, 10), pady=6)
            e = tk.Entry(user_form, font=thai_font(self.fs(24)), bg=ENTRY_BG, fg=TEXT_COLOR,
                         relief="sunken", bd=2, width=34)
            e.grid(row=row, column=1, sticky="w", pady=6)
        ...
        self._toggle_btn = tk.Label(user_form, text="แสดง", font=thai_font(self.fs(20)),
